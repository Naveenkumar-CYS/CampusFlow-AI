...
def _authenticate_as_admin() -> None:
    """Test-suite setup only: /students and /hostel/* are RBAC-protected
    (require_roles), so every request in this file needs a valid bearer
    token -- this file's helpers previously called these endpoints with
    no Authorization header at all, which 401s before ever reaching the
    hostel/event logic under test. Registers a throwaway ADMIN user
    directly via the service layer (no signup endpoint exists) and
    ...
    email = f"testsetup.hostel.{uuid.uuid4().hex[:8]}@example.edu"
