...
def _authenticate_as_admin() -> None:
    """Test-suite setup only: /students and /attendance/records are
    ...
    reaching the attendance/event logic under test. Registers a throwaway
    ADMIN user directly via the service layer (no signup endpoint exists)
    and attaches the resulting token as this module's default
    Authorization header, same token-acquisition pattern as
    test_rbac.py.
    """
    from app.db.session import SessionLocal
    from app.services import auth as auth_service

    email = f"testsetup.attendance.{uuid.uuid4().hex[:8]}@example.edu"
