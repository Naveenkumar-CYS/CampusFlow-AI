...
def _authenticate_as_admin() -> None:
    """Test-suite setup only: /students and /examinations/* are
    ...
    reaching the examination/event logic under test. Registers a
    throwaway ADMIN user directly via the service layer (no signup
    ...
    email = f"testsetup.exam.{uuid.uuid4().hex[:8]}@example.edu"
